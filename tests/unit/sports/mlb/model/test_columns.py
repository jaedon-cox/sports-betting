...
def test_home_side_uses_away_pitching_and_home_batting() -> None:
    ...
    assert inputs.off_wrc_plus_z == pytest.approx(
        (ROW["home_off_wrc_plus"] - WRC_PLUS_LEAGUE_AVG) / WRC_PLUS_SCALE
    )
    assert inputs.opp_starter_siera_z == pytest.approx(
        (ROW["away_starter_siera"] - SIERA_LEAGUE_AVG) / SIERA_SCALE
    )
    assert inputs.opp_bullpen_xfip_z != 0.0
    assert inputs.opp_bullpen_fatigue_raw == ROW["away_bullpen_fatigue"]
    assert inputs.opp_starter_csw_pct == ROW["away_starter_csw_pct"]
    assert inputs.opp_starter_gb_pct == ROW["away_starter_gb_pct"]
...
def test_away_side_uses_home_pitching_and_away_batting() -> None:
    ...
    assert inputs.opp_starter_csw_pct == ROW["home_starter_csw_pct"]
    assert inputs.opp_starter_gb_pct == ROW["home_starter_gb_pct"]
...
def test_missing_column_raises_keyerror() -> None:
    incomplete = ROW.drop("away_starter_siera")
